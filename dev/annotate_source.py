  ...
  def tag_head(self):
    ...
    if self.__is_head_current():
      if self.__force_rebuild:
        # __current_version.tag has a '-<build> at the end.
        last_dash = self.__current_version.tag.rindex('-')
        self.__tag_head_with_build(self.__current_version.tag[:last_dash])
