...
class Builder(object):
  ...
  def start_candidate_release(self, name):
    ...
    This function runs a 'candidate' release using the last git tag as the package
    version and the Bintray configuration passed through arguments. A 'candidate'
    release builds the source, packages the debian and jar files, and publishes
    those to the respective Bintray '$org/$repository'.

    The 'candidate' gradle release task throws a 409 if the package we are trying to
    publish already exists. We'll publish unique package version using build
  ...
  def __do_build(self, subsys):
    try:
      self.start_candidate_release(subsys).check_wait()
