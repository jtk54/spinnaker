...
class BomPromoter(BomGenerator):
  ...
  def push_branch_and_tags(self):
    ...
    for comp in COMPONENTS:
      ...
      version_tag_with_build = ''
      if comp == 'spinnaker-monitoring':
        version_tag_with_build = 'version-{0}'.format(self.__bom_dict[SERVICES]['monitoring-daemon'][VERSION])
      else:
        version_tag_with_build = 'version-{0}'.format(self.__bom_dict[SERVICES][comp][VERSION])

      last_dash = version_tag_with_build.rindex('-')
      version_tag = version_tag_with_build[:last_dash]
